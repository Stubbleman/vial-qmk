...
#include "quantum/split_common/transaction_id_define.h"
...
void keyboard_post_init_user(void) {
    debug_enable = true;
    debug_matrix = true;
    transaction_register_rpc(SPLIT_TRANSACTION_IDS_USER, user_sync_a_slave_handler);
...
void housekeeping_task_user(void) {
    if (is_keyboard_master()) {
        ...
        if (timer_elapsed32(last_sync) > 10) { // 60hz
            ...
            if (transaction_rpc_recv(SPLIT_TRANSACTION_IDS_USER, sizeof(s2m), &s2m)) {
